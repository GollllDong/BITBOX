...
function buildCalender(){ //캘린더를 생성하는 함수 생성
    ...
    if(thisFirst.getDay()!=0){ //만약 이번 월의 첫째날이 일요일이 아니라면(0은 일요일을 의미)
        for(let i = 0; i < thisFirst.getDay(); i++){ //일요일부터 이번 월의 요일까지 날짜를 구하기 위한 for문
            dates.unshift(prevLast.getDate()-i); //이전 월의 마지막 날짜부터 1씩 빼가며 unshift(배열 앞에 값을 넣는다.)
        }
    }
...
function prevCal(){
    ...
    buildCalender();    //buildCalender 함수를 다시 돌린다.
}
